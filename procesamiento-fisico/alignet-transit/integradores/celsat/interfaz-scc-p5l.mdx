...
- incluir `additionalFields` cuando corresponda enviar el desglose de importes y el identificador de turno o vía;
...
<Steps>
  ...
  <Step title="Enviar la solicitud">
    Ejecuta `POST /api/v1/payments` con los tres campos requeridos y, cuando corresponda, el objeto opcional `additionalFields`.
  </Step>
  <Step title="Procesar la respuesta">
    Lee `success`, `resultCode`, `resultMessage` y `result` sin asumir que `success` representa aprobación. Conserva `result.additionalFields` cuando exista.
  </Step>
