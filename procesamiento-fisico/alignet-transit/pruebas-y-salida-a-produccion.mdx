...
| Cancelación remota | `POST /cancel` sobre una autorización activa devuelve HTTP `200` con resultado final `CANCELLED` o `DENIED`. | Confirmar la cancelación solo con `CANCELLED`; `DENIED` no la confirma. |
| Cancelación pendiente | `POST /cancel` devuelve HTTP `202` e incluye `Retry-After`. | Mantener la operación abierta; el mecanismo de recuperación está pendiente de confirmar con Alignet. |
...
<Check>
  El sistema central ejecuta satisfactoriamente autorización, consulta, cancelación, extorno y consulta de extorno.
</Check>
...
<Check>
  Las claves de `additionalFields` están definidas por el comercio, y las monedas y medios de pago están confirmados.
</Check>
