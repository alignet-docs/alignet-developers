...
| El sistema central pierde la respuesta de autorización | Timeout menor que la duración del cobro o pérdida de red. | No inicies otro cobro. Consulta el `operationNumber` original mediante `GET /payments/{operationNumber}`. |
| `202 PENDING` en autorización o extorno | Venció la espera, pero la operación puede seguir activa. | Usa el GET correspondiente y respeta `Retry-After`. |
| `202` en cancelación | Venció la espera de `/cancel`, sin resultado final. | Respeta `Retry-After`; no uses un GET de extorno. El mecanismo de recuperación está pendiente de confirmar. |
...
El siguiente flujo corresponde a `POST /authorize`. Para una cancelación, conserva la referencia y aplica el [procedimiento de `/cancel`](/procesamiento-fisico/alignet-transit/cancelacion#manejo-de-202-y-perdida-de-conexion); no uses `GET /reversals/{operationNumber}`.
