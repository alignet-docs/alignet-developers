...
Usa el código HTTP, `status`, `resultCode` y `operationNumber` para diagnosticar cada caso. No uses `resultMessage` como única base para la lógica del SCC.
...
| `404` al autorizar | El SCC llama a una ruta anterior o incorrecta. | Usa exactamente `POST /authorize`; no uses `/payment` ni `/api/v1/payments`. |
...
| El SCC pierde la respuesta | Timeout menor que la duración del cobro o pérdida de red. | No inicies otro cobro. Consulta el `operationNumber` original. |
...
2. Confirma que el SCC usa esos valores y no una dirección anterior.
...
7. Ejecuta nuevamente `GET /health` desde el mismo host y proceso de red que usará el SCC.
...
- `operationNumber`: String, entre 1 y 64 caracteres y sin espacios;
- `amount`: número entero o cadena numérica, positivo, de 1 a 12 dígitos y en unidades menores;
- `currency`: si se incluye, número o cadena de exactamente tres dígitos;
...
Un timeout describe la conexión del SCC, no el resultado financiero.
...
El límite máximo de consulta y la decisión operativa de la barrera deben definirse antes de producción.
...
- identificador del SCC, carril y Wiseasy P5L;
