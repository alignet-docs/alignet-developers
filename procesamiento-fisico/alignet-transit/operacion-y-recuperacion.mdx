...
Este procedimiento corresponde a `POST /authorize` y su consulta mediante `GET /payments/{operationNumber}`. Para `POST /cancel`, revisa el [manejo específico de `202` y pérdida de conexión](/procesamiento-fisico/alignet-transit/cancelacion#manejo-de-202-y-perdida-de-conexion), porque no existe un GET de cancelación confirmado.
