...
| --- | --- | --- |
| SCC ↔ Componente Local | Disponibilidad, inicio, seguimiento, consulta, cancelación y resultado. | Transporte, IP, puerto, autenticación y timeout coordinados por ambiente. |
...
| PaymePOS SDK ↔ Alignet Transit | Procesamiento y trazabilidad. | HTTPS con TLS 1.3; host y credenciales suministrados por ambiente. |
| SCC ↔ TMC | Registro del evento y conciliación. | Administrado por CELSAT. |

<Note>
  La disponibilidad de la LAN no garantiza la disponibilidad WAN. El SCC debe tratar cada condición conforme a la política operativa coordinada.
</Note>
...
| `tollPointId` | CELSAT | Punto de peaje. | Código estable del maestro acordado. |
| `laneId` | CELSAT | Carril asociado al peaje. | Código estable dentro de `tollPointId`. |
| `TID` | Alignet | Terminal P5L. | Se asocia al comercio, punto de peaje y carril configurados. |
...
Los registros deben permitir reconstruir la secuencia sin incluir credenciales, PAN completo, datos EMV sensibles ni material criptográfico.

## Consideraciones de red

- La comunicación entre el SCC y el P5L utiliza la LAN del carril.
- La configuración puede usar direccionamiento fijo o el esquema acordado con CELSAT.
- VLAN, gateway, DNS, proxy, firewall y allowlists dependen de la infraestructura del ambiente.
- El terminal requiere salida WAN segura hacia los hosts de Alignet.
- SCC y P5L deben mantener sus relojes sincronizados.
- Un terminal se asocia de forma inequívoca con el comercio, punto de peaje, carril y TID configurados.
