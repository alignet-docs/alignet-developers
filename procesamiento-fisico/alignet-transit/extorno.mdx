...
El API de cancelación y extorno permite detener o revertir una operación iniciada previamente mediante `POST /authorize`.
...
En ambos casos, el SCC envía el mismo `operationNumber` utilizado en la autorización original.

<ParamField path="POST /reversals" type="endpoint" />

<Note>
  El SCC no elige la acción mediante un parámetro. Pay-me PinPAD evalúa el estado que tenga la operación cuando procesa `POST /reversals` y aplica la transición correspondiente.
</Note>
...
<Info>
  Este endpoint pertenece a la interfaz local SCC–Pay-me PinPAD. No corresponde a `DELETE /charges` del API de e-commerce.
</Info>
...
| --- | --- | --- | --- | --- |
| `operationNumber` | String | Sí | Entre 1 y 64 caracteres, sin espacios. | Referencia original de `POST /authorize`. Identifica la operación que se cancelará o extornará. |
...
```bash
curl -X POST http://192.168.68.121:8080/reversals \
  -H "Content-Type: application/json" \
  --max-time 110 \
  -d '{"operationNumber":"44992528"}'
```
...
| HTTP | Resultado | Significado | Acción del SCC |
| --- | --- | --- | --- |
...
La guía proporciona **90 segundos** como espera interna de referencia en desarrollo. Configura `POST /reversals` con al menos **100 a 110 segundos** para que el timeout del SCC sea mayor, tanto para cancelación como para extorno.

<Warning>
  El timeout del SCC describe la conexión, no el resultado de la cancelación o el extorno. Consulta `GET /reversals/{operationNumber}` antes de reenviar o tomar una decisión financiera.
</Warning>
...
<CardGroup cols={2}>
  ...
  <Card title="Operación y recuperación" icon="rotate" href="/procesamiento-fisico/alignet-transit/operacion-y-recuperacion">
    Aplica las reglas generales de persistencia, timeout y recuperación del SCC.
  </Card>
</CardGroup>
